...
logger = logging.getLogger("telnyx-gemini-bridge")
...
async def telnyx_webhook(request: Request):
    ...
    try:
        ...
        if event_type == "call.answered":
            ...
            # Retry loop to handle Cloudflare Tunnel DNS propagation delay
            for attempt in range(4):
                ...
                if response.status_code == 200:
                    ...
                    logger.info("⚠️ Got 422, waiting 1.5 seconds for DNS propagation before retrying...")
...
async def media_stream_endpoint(websocket: WebSocket):
    ...
    try:
        async with websockets.connect(gemini_url) as gemini_ws:
            ...
            # Send initial setup request to configure Gemini Live session
            setup_message = {
                "setup": {
            ...
            logger.info("Sent session setup configuration to Gemini.")

            # CRITICAL: Wait for setupComplete before streaming any audio
            setup_confirmed = False
            for _ in range(10):
                try:
                    msg = await asyncio.wait_for(gemini_ws.recv(), timeout=5.0)
                    data = json.loads(msg)
            ...
            # Keep track of audio conversion states
            to_gemini_state = None
            from_gemini_state = None
            
            # Counter for logging packet flow metrics
            packets_to_gemini = 0
            packets_from_gemini = 0

            async def handle_telnyx_to_gemini():
                nonlocal to_gemini_state, packets_to_gemini
                try:
                    # Non-blocking receive text loop
                    while True:
                        message = await websocket.receive_text()
                        data = json.loads(message)
                        event = data.get("event")
                        logger.info(f"Received Telnyx WebSocket event: {event}")
                        
                        if event == "media":
                            payload_b64 = data["media"]["payload"]
                            ulaw_audio = base64.b64decode(payload_b64)
                            
                            # 1. Convert 8kHz G.711 mu-law audio to 8kHz 16-bit linear PCM
                            ...
                            # 3. Base64 encode and send to Gemini Live
                            gemini_payload = {
                                "realtime_input": {
                                    "audio": {
                            ...
                            packets_to_gemini += 1
                            if packets_to_gemini % 50 == 0:
                                logger.info(f"🎤 Forwarded {packets_to_gemini} audio packets to Gemini")
                                
                except Exception as e:
                    logger.error(f"Error in Telnyx -> Gemini stream: {repr(e)}")

            async def handle_gemini_to_telnyx():
                nonlocal from_gemini_state, packets_from_gemini
                try:
                    # Non-blocking async loop to read from Gemini
                    async for message in gemini_ws:
                        response_data = json.loads(message)
                        
                        # Extract the audio payload from Gemini model output
                        # Gemini returns camelCase keys (serverContent, modelTurn, inlineData)
            ...
            # Wait until one of the tasks finishes (e.g. call hung up or connection closed)
